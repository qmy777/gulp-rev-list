...
plugin.manifest = function (pth, opts) {
	...
	return through.obj(function (file, enc, cb) {
		...
		var otherPath = path.resolve(file.base) + '/';
		var relativeFile = originalFile.replace(otherPath,'');

		manifest[relativeFile] = file.revHash;
		...
		getManifestFile(opts, function (err, manifestFile) {
			...
			var jsonStr ='window.'+opts.winval+' = '+JSON.stringify(sortKeys(manifest))+';';
			manifestFile.contents = new Buffer(jsonStr, null, '  ');
